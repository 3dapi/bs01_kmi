...
--1. 고정된 그림 화면출력
--2. 버튼 화면 출력 (button.csv)
--3. 버튼 기능 구현	(u_button.lua)
--4. 마우스 콘트롤(u_control.lua)
--5. 타이머 콘트롤(u_timer.lua)
--6. 움직이는 그림 출력



window = {w = 480, h = 320}		--윈도 width/height
max_texture = 8				--일반텍스처 갯수
...
function SetFont(_fnt, _str, _px, _py)
	_fnt.ct = _fnt.ct + 1
	Lfont.Setup(_fnt[_fnt.ct], _str, _px, _py, "0xFFFF1111")
end
